...
class ProductTypeTaxRepository {
    ...
    public function getById($id_tax, $id_product_type) {
        
        $statement = $this->database->getConnection()
        ->prepare(" SELECT id_product_type, id_tax
        FROM product_type_tax 
        WHERE id_tax = :id_tax and id_product_type = :id_product_type ");

        $statement->bindParam(':id_product_type', $id_product_type);
        $statement->bindParam(':id_tax', $id_tax);
        $statement->execute();
